...
public class Expr$Exp extends SimpleExpression<Double> {
    ...
    public String toString(@Nullable Event event, boolean b) {
        return "(%string%'s job exp|job exp of %string%)";
    }

    @Nullable
    protected Double[] get(Event event) {
        String uuid = (String)this.uuid.getSingle(event);
        return new Double[] {Double.valueOf(DayJob.getExp(uuid))};
    }
    ...
    public void change(Event event, Object[] d, Changer.ChangeMode mode) {
        ...
        if (mode == Changer.ChangeMode.ADD) {
            DayJob.addExp(uuid, (int) num);
        } else if (mode == Changer.ChangeMode.REMOVE) {
            DayJob.addExp(uuid, (int) -num);
        } else if (mode == Changer.ChangeMode.SET) {
            DayJob.setExp(uuid, (int) num);
        }
    }
